...
<a href="printAllProjects.php">Preview All</a>
...
if ($num > 0) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        extract($row);
        echo "<tr>";
        echo "<td class='col-md-4'><a href='singleProjectDetails.php?contractNo=$contractNo&name=$projectName'>{$projectName}</a></td>";               
        echo "<td>{$client}</td>";
        echo "<td>{$progress}</td>";
